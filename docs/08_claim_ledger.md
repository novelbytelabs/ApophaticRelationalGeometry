...
This ledger prevents definitions, software implementation, numerical observations, scientific evidence, consensus findings, and philosophical interpretations from being presented at the same evidential level.
...
- **Consensus C4** — established within explicit mathematical or scientific scope.
- **Consensus C3** — strong convergence within explicit scope.
- **Interpretively disputed C2** — accepted result with live disagreement about meaning or ontology.
- **Plausible synthesis C1** — compatible cross-field synthesis not independently established.
- **Open or speculative C0** — conjecture or unresolved question.
...
- **Rejected** — disproved, contradicted, withdrawn, or superseded.
...
| ARG-C016 | Local influence normally dominates away from coherence boundaries. | C0 conjecture | Frozen regimes and matched comparisons. | Not established. |
| ARG-C017 | Projection correction grows near admissibility boundaries. | C0 conjecture | Boundary definition and confirmatory comparison. | Not established. |
| ARG-C018 | Feedback magnitude grows near organization-dependent transitions. | C0 conjecture | $M_0/M_F$ comparisons and interventions. | Not established. |
| ARG-C019 | Coevolving geometry produces behavior not reducible to adaptive edge weights alone. | C0 conjecture | Identifiability proof or discriminating experiment. | Candidate contribution. |
| ARG-C020 | The full family is not observationally equivalent to an established constrained adaptive network. | C0 conjecture | Primary-source mapping and reparameterization audit. | Candidate novelty. |
...
| ARG-C026 | Objective reality has a pluralistic-monist organization. | Philosophical / C0 | No direct evidence. | Ontological conjecture. |
...
| ARG-C028 | A prime-generated substrate underlies the computational fabric. | C0 and quarantined | Formal object, lawful dynamics, observable, and unique prediction. | Separate branch. |
...
| ARG-C032 | Any ARG model has superior scientific explanatory power. | Unverified | Frozen comparison, matched alternatives, uncertainty, held-out confirmation, and residual consensus gate. | No scientific result. |
...
| ARG-C042 | Same-state equality $f_{FP}=f_P$ implies identical $M_P$ and $M_{FP}$ trajectories. | Rejected | $s/q$ dynamics differ and may alter later proposals. | Not licensed. |
...
| ARG-C044 | The four-model comparative experiment establishes a mechanism result. | Unverified | Integrity remediation, consensus residual gate, frozen execution, pilot, held-out confirmation, and claim review. | No experiment executed. |
...
| ARG-C049 | A development pilot is authorized in principle. | Superseded / blocked | Integrity and consensus residual gates now both required. | No current execution permission. |
| ARG-C050 | The development pilot has been executed. | Unverified | Completed authorized pilot archive. | Not executed. |
| ARG-C051 | The Phase 6 runner reconstructs exactly the frozen pilot configurations and rejects confirmatory/mixed input. | Implemented and unit-tested, audit remediation pending | Independent membership/hash reconstruction and tripwires. | Runner software only. |
| ARG-C052 | The runner implements frozen integration, controls, relabeling, and numerical-floor policies. | Implemented and unit-tested, audit remediation pending | Smoke/reference parity, known-answer gates, and hosted verification. | Runner software only. |
| ARG-C053 | The runner produces a write-once, checksummed archive and independently verifiable hashes. | Implemented and unit-tested, audit remediation pending | Audit defects must be remediated and reverified. | Archive software only. |
| ARG-C054 | Phase 6 runner verification produced pilot evidence. | Rejected | No execution authorization or pilot archive exists. | No pilot data. |
...
| ARG-C056 | A realized trajectory depends on a transition law and initial condition. | Consensus C4 within specified dynamical models | Schmid 2007 and standard initial-value theory. | Established neighboring result; not ARG novelty. |
| ARG-C057 | Different states can amplify, decay, or cancel differently under the same operator. | Consensus C4 in modal and nonmodal dynamics | Schmid 2007. | Established neighboring result; not ARG novelty. |
| ARG-C058 | State and topology may coevolve. | Consensus C4 in adaptive-network science | Gross and Blasius 2008. | Established architecture; not ARG novelty. |
| ARG-C059 | Nonlinear persistence has multiple inequivalent forms including basin, manifold, metastable, and structural persistence. | Consensus C4 within scoped mathematics | Menck et al.; Fenichel; Froyland and Padberg; Smale. | Bare `persistence` is prohibited. |
| ARG-C060 | Reachability and observability depend on dynamics, input/output maps, state, and horizon. | Consensus C4 in linear theory; C3–C4 in nonlinear local theory | Kalman 1963; Moore 1981; Hermann and Krener 1977. | Broad state–system interaction is established. |
| ARG-C061 | Balanced realization provides an established joint controllability–observability importance measure in stable minimal LTI systems. | Consensus C4 within stated assumptions | Moore 1981. | Any ARG compatibility quantity must be compared against Gramian/Hankel measures. |
| ARG-C062 | Viability, positive invariance, and controlled invariance are distinct established preservation predicates. | Consensus C4 within control theory | Saint-Pierre 1994; Blanchini 1999. | Autonomous and corrected persistence must be separated. |
| ARG-C063 | Smoothly evolving state spaces can be related by explicit pushforward/pullback maps satisfying consistency laws. | Consensus C4 within the audited framework | Alphonse, Elliott, and Stinner 2015. | General transport requirement is established. |
| ARG-C064 | Source and destination spaces alone do not determine all transport, control, observation, or preservation semantics. | Consensus C3–C4 within audited frameworks | Evolving-space and control source baskets. | Typed context must be explicit. |
| ARG-C065 | `State–structure compatibility` denotes one universal invariant or scalar. | C1 / unverified and currently disfavored | Current evidence supports a typed family, not one quantity. | Not licensed. |
| ARG-C066 | A bare occurrence pair $(B_t,x_t)$ is sufficient for every scientific persistence or compatibility claim. | Rejected as a general methodological claim | Control theory requires inputs, outputs, constraints, controls, disturbances, observables, and horizons for many questions. | Retain only for scoped autonomous models. |
| ARG-C067 | A typed scientific context may require $(\mathcal X,F,B_{in},C_{out},K,\mathcal U,\mathcal W,\Phi,T)$. | Preliminary methodological synthesis C1–C3, component-dependent | First three CS1 baskets. | Typing aid, not ontology; final schema pending. |

## Claim promotion rules

A claim may move to **numerically supported** only when its contract, protocol, baselines, configurations, solver policy, raw outputs, exploratory/confirmatory separation, and scientific residual are frozen and execution has occurred.
...
A claim may move to **Consensus C3 or C4** only after completed source notes, scope and exception review, translation audit, and independent verification.
...
- ARG discovered state–structure interaction;
- ARG discovered that persistence depends on compatibility;
- persistence is one universal quantity;
- a structure–state pair is representation-independent ontology;
- ARG is a fundamental theory of physics;
- primes are the ontological foundation of reality.

## Maintenance rule

Every theorem, implementation, protocol amendment, experiment, conceptual extension, or consensus finding must update this ledger. No claim may exceed `13_alignment_and_claim_ceiling.md` or the active consensus grade.
