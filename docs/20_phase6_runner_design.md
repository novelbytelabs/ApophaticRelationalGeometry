...
Phase 6 implements the execution and archival machinery for the frozen Phase 5 protocol. This document describes the runner gate; it does not report pilot data.

Current authorization remains:

$$
\boxed{
\text{development pilot authorized in principle};
\quad
\text{runner verification required before execution};
\quad
...
A later execution authorization must name a previously verified `runner_source_commit`. The authorization may be committed afterward without creating a self-referential commit hash, but all protected runner and frozen-protocol files must remain unchanged between that runner commit and the execution commit.
...
- DOP853 replication for $M_F$ and $M_P$;
...
The runner computes the frozen configuration-level numerical floor
...
The independent test path does not call the production planner, archive writer, checksum writer, or summary function it is checking.
...
- numerical step times and indices.

Configuration summaries name every raw input hash and include:

- primary $M_F$–$M_P$ full-state effect;
- configuration-level numerical floor;
- developmental H2–H4 effects;
- geometry differences;
- feedback and projection ratios;
- numerical acceptance results;
- permutation tripwire results.
...
Passing Phase 6 runner verification permits only:
...
It does not license any scientific conclusion.
